...
You can install circuit-tracer with dev dependencies with `pip install -e ".[dev]"`. This project uses `pytest` for tests, `ruff` for linting and formatting, and `pyright` for type-checking. Before contributing a PR, please run the following to make sure your code passes all checks:
...
If you are using VSCode or Cursor, you can install the ruff plugin, and the add the following to your `.vscode/settings.json` to enable auto-formatting with ruff on save:
