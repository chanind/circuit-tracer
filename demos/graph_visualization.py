    ...
    def set_node_activation_fractions(self, current_activations):
        for node in self.nodes.values():
            if node.features:
                ...
                node.activation = (
                    (current_node_activation / node.default_activations).mean().item()  # type: ignore
                )
            else:
...
def create_graph_visualization(
    ...
    node_data = calculate_node_positions(nodes)  # type: ignore

    # Build connection data
    connections = build_connections_data(nodes)  # type: ignore
