...
def test_bridge_gpt2_replacement_model_behaves_like_legacy_replacement_model(
    ...
    legacy_model = LegacyReplacementModel.from_pretrained_and_transcoders(
        "gpt2", gpt2_transcoder_set
    )
    bridge_model = ReplacementModel.from_hf_model(hf_model, hf_tokenizer, gpt2_transcoder_set)
...
def test_TransformerBridge_gpt2_behaves_like_HookedTransformer_gpt2():
    ...
    """
    legacy_gpt2 = HookedTransformer.from_pretrained_no_processing("gpt2")
    bridge_gpt2: TransformerBridge = TransformerBridge.boot_transformers("gpt2")  # type: ignore
    bridge_gpt2.enable_compatibility_mode()
